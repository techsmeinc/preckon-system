...
let _idc = 0;
export const newId = () => `e${++_idc}`;
export function withIds(m) {
    let changed = false;
...
export function mapEntityPoints(e, fn) {
    if (e.kind === "line") {
    ...
    if (e.kind === "poly")
        return { ...e, pts: e.pts.map((p) => fn(p.x, p.y)) };
    const p = fn(e.x, e.y);
    return { ...e, x: p.x, y: p.y };
}
export const translateEntity = (e, dx, dy) => mapEntityPoints(e, (x, y) => ({ x: x + dx, y: y + dy }));
export const rotateEntity = (e, cx, cy, ang) => {
...
export function scaleEntityAbout(e, cx, cy, f) {
...
export function mirrorEntity(e, ax, ay, bx, by) {
 ...
 */
export function hatchSegments(poly, spacing, angle) {
    if (poly.length < 3 || spacing <= 0)
        return [];
    ...
    const back = (x, y) => ({ x: x * cosB - y * sinB, y: x * sinB + y * cosB });
    const out = [];
    const n = rot.length;
    for (let y = minY + spacing / 2; y < maxY; y += spacing) {
        ...
        for (let i = 0; i + 1 < xs.length; i += 2)
            out.push([back(xs[i], y), back(xs[i + 1], y)]);
        if (out.length > 20000)
            break; // safety on pathological boundaries
    }
    return out;
}
// ── reading a DXF ───────────────────────────────────────────────────────────
...
export function parseToModel(dxf) {
    ...
    for (const e of dxf?.entities ?? []) {
        ...
        if (type === "LINE") {
            ...
            const mesh = Boolean(e.polyfaceMesh ?? e.is3dPolygonMesh);
    ...
    const tableLayers = dxf?.tables
        ?.layer?.layers ?? {};
    const layers = [];
    const addLayer = (name, aci, visible) => {
        if (!layers.some((l) => l.name === name))
            layers.push({ name, aci, visible });
    };
    for (const [name, def] of Object.entries(tableLayers))
        addLayer(name, num(def.colorIndex ?? def.color, 7) || 7, def.visible !== false);
    for (const n of layerNames)
        addLayer(n, 7, true);
    return { layers, entities, insunits };
}
/** Bounding box of all geometry. */
export function modelBounds(m) {
 ...
 */
export function robustBounds(m) {
    const xs = [], ys = [];
    for (const e of m.entities) {
        if (e.kind === "line") {
            xs.push(e.x1, e.x2);
...
export function serializeModel(m) {
    ...
    for (const e of m.entities) {
        if (e.kind === "line") {
            ...
            for (const p of e.pts)
                out.push("0", "VERTEX", "8", e.layer, "10", f(p.x), "20", f(p.y));
            out.push("0", "SEQEND");
        }
        else {
...
// ── units ───────────────────────────────────────────────────────────────────
export const UNIT_LABEL = { 1: "in", 2: "ft", 4: "mm", 5: "cm", 6: "m" };
export const UNIT_MM = { mm: 1, cm: 10, m: 1000, in: 25.4, ft: 304.8 };
export const UNIT_OPTIONS = ["mm", "cm", "m", "in", "ft"];
export const nativeUnit = (insunits) => UNIT_LABEL[insunits] ?? "mm";
/** How many display units one drawing unit is worth. */
export function unitFactor(insunits, display) {
