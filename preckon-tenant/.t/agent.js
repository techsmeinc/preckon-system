...
function sanitise(raw, digest) {
    ...
    for (const o of Array.isArray(raw?.ops) ? raw.ops : []) {
        ...
        else if (o.op === "delete_region" && [o.x1, o.y1, o.x2, o.y2].every((v) => num(v) !== null)) {
            ops.push({ op: "delete_region", x1: o.x1, y1: o.y1, x2: o.x2, y2: o.y2 });
        }
    }
    const marks = [];
    for (const m of Array.isArray(raw?.marks) ? raw.marks : []) {
        if (!KNOWN_MARKS.has(m?.kind))
            continue;
        const label = typeof m.label === "string" ? m.label.slice(0, 80) : undefined;
        if (m.kind === "dot" && num(m.x) !== null && num(m.y) !== null)
            marks.push({ kind: "dot", x: m.x, y: m.y, label });
        else if (m.kind === "edge" && [m.x1, m.y1, m.x2, m.y2].every((v) => num(v) !== null)) {
            marks.push({ kind: "edge", x1: m.x1, y1: m.y1, x2: m.x2, y2: m.y2, label });
        }
        else if (m.kind === "area" && Array.isArray(m.pts) && m.pts.length >= 3) {
            marks.push({ kind: "area", pts: m.pts.filter((p) => num(p?.x) !== null && num(p?.y) !== null).slice(0, 400), label });
        }
    }
    return { ops: ops.slice(0, 200), marks: marks.slice(0, 60) };
}
export async function runCadAgent({ question, digest, describe, layerGeometry, model, callAnthropic, }) {
    const messages = [{
            role: "user",
    ...
    for (; step < MAX_STEPS; step++) {
        const res = await callAnthropic({ model, system: SYSTEM, messages, tools: [TOOL], maxTokens: 3000 });
    ...
    if (!answer)
        answer = "I could not read anything from this drawing that answers that.";
...
export function applyCadOps(m, ops) {
    let entities = [...m.entities];
    const layers = [...m.layers];
    const ensure = (name, aci = 1) => {
        if (!layers.some((l) => l.name === name))
            layers.push({ name, aci, visible: true });
    };
    let added = 0, removed = 0;
    const push = (e) => { entities.push({ ...e, id: newId() }); added++; };
    for (const o of ops) {
        switch (o.op) {
            case "add_line":
                ensure(o.layer);
                push({ kind: "line", layer: o.layer, x1: o.x1, y1: o.y1, x2: o.x2, y2: o.y2 });
                break;
            case "add_rect":
                ensure(o.layer);
            ...
            case "add_poly":
                ...
                push({ kind: "poly", layer: o.layer, closed: !!o.closed, pts: o.pts });
                break;
            case "add_text": {
                ensure(o.layer, 2);
