...
import { hatchSegments, mirrorEntity, modelBounds, newId, robustBounds, rotateEntity, scaleEntityAbout, translateEntity, } from "./model";
const DRAW_TOOLS = ["line", "polyline", "rect", "circle", "text", "dimension", "hatch"];
const MODIFY_TOOLS = ["move", "copy", "rotate", "scale", "mirror"];
export const isDraw = (t) => DRAW_TOOLS.includes(t);
export const isModify = (t) => MODIFY_TOOLS.includes(t);
export const ALL_SNAP_MODES = ["end", "mid", "center", "intersection", "perp", "nearest", "node"];
export const DEFAULT_SNAPS = ["end", "mid", "center", "intersection", "perp", "node"];
const SNAP_PRI = { end: 1, intersection: 2, mid: 3, perp: 4, center: 5, node: 6, nearest: 7 };
...
const aciHex = (aci) => ACI_SCREEN[aci] ?? "#dfe4ea";
...
function entitySegments(e) {
    ...
    if (e.kind === "poly") {
        ...
        for (let i = 0; i < e.pts.length - 1; i++)
            out.push(e.pts[i].x, e.pts[i].y, e.pts[i + 1].x, e.pts[i + 1].y);
        if (e.closed && e.pts.length > 2)
            out.push(e.pts[e.pts.length - 1].x, e.pts[e.pts.length - 1].y, e.pts[0].x, e.pts[0].y);
        return out;
    }
    return [];
}
const closestOnSeg = (px, py, x1, y1, x2, y2) => {
    const dx = x2 - x1, dy = y2 - y1, l2 = dx * dx + dy * dy;
    let t = l2 ? ((px - x1) * dx + (py - y1) * dy) / l2 : 0;
    t = Math.max(0, Math.min(1, t));
    return { x: x1 + t * dx, y: y1 + t * dy };
};
function footOnSeg(px, py, x1, y1, x2, y2) {
    const dx = x2 - x1, dy = y2 - y1, l2 = dx * dx + dy * dy;
    if (!l2)
        return null;
...
function distToEntity(e, p) {
    if (e.kind === "text")
        return Math.hypot(p.x - e.x, p.y - e.y);
    const s = entitySegments(e);
    let best = Infinity;
    for (let i = 0; i < s.length; i += 4)
        best = Math.min(best, distToSeg(p.x, p.y, s[i], s[i + 1], s[i + 2], s[i + 3]));
    return best;
}
function entityInRect(e, a, b) {
    ...
    if (e.kind === "poly")
        return e.pts.some((p) => inside(p.x, p.y));
 ...
 */
export function constrainAngle(base, raw, opts) {
...
/* ── the viewport ────────────────────────────────────────────────────────── */
export const CadViewport = forwardRef(function CadViewport({ model, mono = false, units = "", unitFactor = 1, precision = 2, measuring = false, fitOn, tool = "select", activeLayer = "0", onChange, onOperationDone, onSelectionChange, osnap = true, snapModes = DEFAULT_SNAPS, ortho = false, polar = false, polarInc = 15, onHint, marks = [], }, ref) {
    const hostRef = useRef(null);
    const canvasRef = useRef(null);
    const [cam, setCam] = useState({ ox: 0, oy: 0, scale: 1 });
    const [size, setSize] = useState({ w: 0, h: 0 });
    const [cursor, setCursor] = useState(null);
    const [coords, setCoords] = useState(null);
    const [hover, setHover] = useState(null);
    const [pts, setPts] = useState([]); // measure chain
    const [draft, setDraft] = useState([]); // in-progress draw / modify
    ...
    /* ── projection ──────────────────────────────────────────────────────── */
    const sx = useCallback((x) => size.w / 2 + (x - cam.ox) * cam.scale, [size.w, cam]);
    const sy = useCallback((y) => size.h / 2 - (y - cam.oy) * cam.scale, [size.h, cam]);
    const toScreen = useCallback((p) => ({ x: sx(p.x), y: sy(p.y) }), [sx, sy]);
    const worldAt = useCallback((px, py) => {
    ...
    const snapData = useMemo(() => {
        ...
        const verts = [], segs = [], nodes = [], centers = [];
        for (const e of model.entities) {
            if (hidden.has(e.layer))
                continue;
            if (e.kind === "line") {
    ...
    useEffect(() => {
        ...
        const X = (x) => size.w / 2 + (x - cam.ox) * cam.scale;
        const Y = (y) => size.h / 2 - (y - cam.oy) * cam.scale;
        ...
        if (step * cam.scale > 6) {
            const draw = (major) => {
                ...
                ctx.lineWidth = 1;
                ctx.beginPath();
                const isMajor = (v) => Math.abs(v / (step * 5) - Math.round(v / (step * 5))) < 1e-6;
        ...
        const hidden = new Set(model.layers.filter((l) => !l.visible).map((l) => l.name));
        const aciOf = (layer) => model.layers.find((l) => l.name === layer)?.aci ?? 7;
        ctx.lineWidth = 1;
        ctx.lineCap = "round";
        for (const { layer, arr } of batches.segs) {
            ...
            for (let i = 0; i < arr.length; i += 4) {
                ...
                if ((x1 < wx0 && x2 < wx0) || (x1 > wx1 && x2 > wx1) || (y1 < wy0 && y2 < wy0) || (y1 > wy1 && y2 > wy1))
                    continue;
        ...
        for (const t of batches.texts) {
            if (drawn >= MAX_TEXT)
                break;
            if (hidden.has(t.layer))
                continue;
            const px = t.h * cam.scale;
            if (px < MIN_TEXT_PX)
                continue;
            if (t.x < wx0 || t.x > wx1 || t.y < wy0 || t.y > wy1)
                continue;
        ...
        if (selection.size) {
            ...
            ctx.beginPath();
            for (const e of model.entities) {
                if (!e.id || !selection.has(e.id))
                    continue;
                if (e.kind === "text") {
                    ctx.moveTo(X(e.x) - 4, Y(e.y) - 4);
                    ctx.lineTo(X(e.x) + 4, Y(e.y) + 4);
                    ctx.moveTo(X(e.x) - 4, Y(e.y) + 4);
                    ctx.lineTo(X(e.x) + 4, Y(e.y) - 4);
                    continue;
                }
    ...
    /* ── mutation ────────────────────────────────────────────────────────── */
    const commit = useCallback((entities) => {
        onChange?.({ ...modelRef.current, entities });
    }, [onChange]);
    const NEW_LAYER_ACI = { DIMENSIONS: 4, HATCH: 8, NOTES: 2, MARKUP: 1 };
    const addEntities = useCallback((...added) => {
    ...
    const transformSelected = useCallback((fn, keepOriginal = false) => {
        const sel = selRef.current;
        if (!sel.size)
            return;
        if (keepOriginal) {
     ...
     */
    const buildDimension = useCallback((p1, p2, offPt) => {
    ...
    const hatchToEntities = useCallback((poly) => {
        ...
        return hatchSegments(poly, Math.max(diag / 40, 1e-6), Math.PI / 4)
            .map(([a, b]) => ({ kind: "line", layer: "HATCH", x1: a.x, y1: a.y, x2: b.x, y2: b.y }));
    }, []);
    const finishPolyline = useCallback(() => {
        const d = trimTail(draftRef.current);
        if (d.length >= 2)
            addEntities({ kind: "poly", layer: activeLayer, closed: false, pts: d.map((q) => ({ x: q.x, y: q.y })) });
        setDraft([]);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    const currentBase = useCallback(() => {
        if (draftRef.current.length)
            return draftRef.current[draftRef.current.length - 1];
        if (measuringRef.current && ptsRef.current.length)
            return ptsRef.current[ptsRef.current.length - 1];
        return null;
    }, []);
    const resolveSnap = useCallback((w, base) => {
        if (!osnap)
            return null;
        ...
        if (enabled.has("end"))
            for (let i = 0; i < verts.length; i += 2)
                consider(verts[i], verts[i + 1], "end");
        const near = [];
        if (enabled.has("mid") || enabled.has("nearest") || enabled.has("perp") || enabled.has("intersection")) {
            for (let i = 0; i < segs.length; i += 4) {
                ...
                if (w.x < Math.min(x1, x2) - tol || w.x > Math.max(x1, x2) + tol || w.y < Math.min(y1, y2) - tol || w.y > Math.max(y1, y2) + tol)
                    continue;
                near.push(i);
                if (enabled.has("mid"))
                    consider((x1 + x2) / 2, (y1 + y2) / 2, "mid");
        ...
        if (enabled.has("center"))
            for (let i = 0; i < centers.length; i += 2)
                consider(centers[i], centers[i + 1], "center");
        if (enabled.has("node"))
            for (let i = 0; i < nodes.length; i += 2)
                consider(nodes[i], nodes[i + 1], "node");
        return bestType ? { p: { x: bx, y: by }, type: bestType } : null;
    }, [osnap, snapModes, snapData, worldPerPx]);
    const resolvePoint = useCallback((raw) => {
        ...
        if (sn && !(constrained && sn.type === "nearest"))
            return { p: sn.p, snapped: true, type: sn.type };
    ...
    /* ── tools ───────────────────────────────────────────────────────────── */
    const handleToolClick = useCallback((p, shift) => {
        const t = toolRef.current;
        switch (t) {
            case "select": {
                const id = pickAt(p);
            ...
            case "hatch": {
                const selPolys = modelRef.current.entities.filter((e) => !!e.id && selRef.current.has(e.id) && e.kind === "poly" && e.closed);
                if (selPolys.length) {
                    const add = [];
                    for (const poly of selPolys)
                        add.push(...hatchToEntities(poly.pts));
                    if (add.length)
                        addEntities(...add);
                }
                else
    ...
    const onPointerDown = (e) => {
        ...
        if (e.button === 2 && draftRef.current.length && finishDraft())
            return;
    ...
    const onPointerMove = (e) => {
        ...
        if (d && tool === "select" && !measuring) {
            if (Math.hypot(px - d.x, py - d.y) > 4)
                setSelRect({ x1: d.x, y1: d.y, x2: px, y2: py });
        }
    };
    const onPointerUp = (e) => {
        const d = downRef.current;
        ...
        if (tool === "select" && !measuring && moved > 5) {
            ...
            const hits = modelRef.current.entities.filter((en) => en.id && entityInRect(en, a, b)).map((en) => en.id);
        ...
        setSelRect(null);
        if (moved > 5)
            return; // a drag that was not a window-select places no point
    ...
    useImperativeHandle(ref, () => ({
        fit: fitNow,
        zoom: (factor) => setCam((c) => ({ ...c, scale: Math.max(1e-6, Math.min(1e9, c.scale * factor)) })),
        clearMeasure: () => setPts([]),
        deleteSelection,
        clearSelection: () => setSelection(new Set()),
        selectAll: () => setSelection(new Set(model.entities.map((e) => e.id).filter(Boolean))),
        selectLayer: (name) => setSelection(new Set(model.entities.filter((e) => e.id && e.layer === name).map((e) => e.id))),
        selectionCount: () => selRef.current.size,
    }), [fitNow, deleteSelection, model.entities]);
    /* ── overlay geometry ────────────────────────────────────────────────── */
    const fmt = (v) => (v * unitFactor).toFixed(precision);
    ...
    if (hp && draft.length) {
        if (tool === "line" || tool === "polyline") {
            for (let i = 1; i < draft.length; i++)
                previewSegs.push([draft[i - 1], draft[i]]);
            ...
            if (draft.length === 1)
                ...
                for (const e of buildDimension(draft[0], draft[1], hp))
                    if (e.kind === "line")
                        previewSegs.push([{ x: e.x1, y: e.y1 }, { x: e.x2, y: e.y2 }]);
        }
        else if (tool === "hatch") {
            for (let i = 1; i < draft.length; i++)
                previewSegs.push([draft[i - 1], draft[i]]);
    ...
    const previewScreen = previewSegs.map(([a, b]) => [toScreen(a), toScreen(b)]);
    const ghostScreen = ghostSegs.map(([a, b]) => [toScreen(a), toScreen(b)]);
    ...
    const hint = (() => {
        ...
        if (tool === "select")
            return selection.size ? `${selection.size} selected · Del erases · Esc clears` : "Click to select · drag a window · Shift adds";
        if (tool === "polyline")
            return `Click points · double-click, right-click or Enter finishes${draft.length ? ` · ${draft.length} pts` : ""}`;
        if (tool === "dimension")
            return draft.length < 2 ? "Click the two points to dimension" : "Click to place the dimension line";
        if (tool === "hatch")
            return selection.size ? "Click to hatch the selected closed shape" : draft.length ? `Trace a boundary · double-click or Enter fills · ${draft.length} pts` : "Trace a boundary · double-click fills · or select a shape first";
        if (isDraw(tool))
            return `Draw ${tool} · Esc cancels`;
        if (isModify(tool))
            return selection.size ? (draft.length ? "Click destination" : "Click base point") : "Select something first";
        if (tool === "pan")
            return "Drag to pan · wheel zooms";
        return "";
    })();
    useEffect(() => { onHint?.(hint); }, [hint, onHint]);
    return (_jsxs("div", { ref: hostRef, className: "cadvp", style: { cursor: tool === "pan" ? "grab" : "crosshair" }, onPointerDown: onPointerDown, onPointerMove: onPointerMove, onPointerUp: onPointerUp, onPointerLeave: () => { setCursor(null); if (!measuring)
            setCoords(null); }, onDoubleClick: () => { if (!(draftRef.current.length && finishDraft()))
            fitNow(); }, onContextMenu: (e) => e.preventDefault(), children: [_jsx("canvas", { ref: canvasRef, style: { width: "100%", height: "100%", display: "block" } }), cursor && tool !== "pan" && (_jsxs(_Fragment, { children: [_jsx("div", { className: "cadvp-x", style: { left: cursor.px } }), _jsx("div", { className: "cadvp-y", style: { top: cursor.py } })] })), _jsxs("svg", { className: "cadvp-ov", "aria-hidden": true, children: [mChain.length > 0 && (_jsxs(_Fragment, { children: [_jsx("polyline", { points: mScreen.map((p) => `${p.x},${p.y}`).join(" "), fill: "none", stroke: "#fbbf24", strokeWidth: 1.5, strokeDasharray: "5 3" }), mScreen.map((p, i) => (_jsx("rect", { x: p.x - 3, y: p.y - 3, width: 6, height: 6, fill: BG, stroke: "#fbbf24", strokeWidth: 1.5 }, `v${i}`))), mSeg.map((s, i) => (_jsxs("text", { x: s.x, y: s.y - 5, fill: "#fde68a", fontSize: 11, fontFamily: "monospace", textAnchor: "middle", stroke: BG, strokeWidth: 3, paintOrder: "stroke", children: [fmt(s.d), " ", units] }, `s${i}`)))] })), ghostScreen.map(([a, b], i) => (_jsx("line", { x1: a.x, y1: a.y, x2: b.x, y2: b.y, stroke: HILITE, strokeOpacity: 0.6, strokeWidth: 1 }, `g${i}`))), previewScreen.map(([a, b], i) => (_jsx("line", { x1: a.x, y1: a.y, x2: b.x, y2: b.y, stroke: "#6f8cff", strokeWidth: 1.4, strokeDasharray: "6 3" }, `p${i}`))), marks.map((m, i) => {
                        if (m.kind === "dot") {
                            const p = toScreen(m);
                            return (_jsxs("g", { children: [_jsx("circle", { cx: p.x, cy: p.y, r: 6, fill: "#f59e0b", fillOpacity: 0.9, stroke: BG, strokeWidth: 1.5 }), m.label && (_jsx("text", { x: p.x + 10, y: p.y + 4, fill: "#fbbf24", fontSize: 12, fontFamily: "monospace", stroke: BG, strokeWidth: 3, paintOrder: "stroke", children: m.label }))] }, `m${i}`));
                        }
                        if (m.kind === "edge") {
                            const a = toScreen({ x: m.x1, y: m.y1 });
                            const b = toScreen({ x: m.x2, y: m.y2 });
                            return (_jsxs("g", { children: [_jsx("line", { x1: a.x, y1: a.y, x2: b.x, y2: b.y, stroke: "#f59e0b", strokeWidth: 3, strokeOpacity: 0.85 }), _jsx("circle", { cx: a.x, cy: a.y, r: 4, fill: "#f59e0b" }), _jsx("circle", { cx: b.x, cy: b.y, r: 4, fill: "#f59e0b" }), m.label && (_jsx("text", { x: (a.x + b.x) / 2, y: (a.y + b.y) / 2 - 8, fill: "#fbbf24", fontSize: 12, fontFamily: "monospace", textAnchor: "middle", stroke: BG, strokeWidth: 3, paintOrder: "stroke", children: m.label }))] }, `m${i}`));
                        }
                        const pts = m.pts.map(toScreen);
                        const cx = pts.reduce((s2, p) => s2 + p.x, 0) / (pts.length || 1);
                        const cy = pts.reduce((s2, p) => s2 + p.y, 0) / (pts.length || 1);
                        return (_jsxs("g", { children: [_jsx("polygon", { points: pts.map((p) => `${p.x},${p.y}`).join(" "), fill: "#f59e0b", fillOpacity: 0.14, stroke: "#f59e0b", strokeWidth: 2 }), m.label && (_jsx("text", { x: cx, y: cy, fill: "#fde68a", fontSize: 13, fontFamily: "monospace", textAnchor: "middle", stroke: BG, strokeWidth: 4, paintOrder: "stroke", children: m.label }))] }, `m${i}`));
                    }), trackScreen && hoverScreen && (_jsx("line", { x1: trackScreen.x, y1: trackScreen.y, x2: hoverScreen.x, y2: hoverScreen.y, stroke: "#a3e635", strokeWidth: 1, strokeDasharray: "4 4" })), hoverScreen && hover && _jsx(SnapGlyph, { type: hover.type, x: hoverScreen.x, y: hoverScreen.y })] }), selRect && (_jsx("div", { className: "cadvp-sel", style: {
                ...
                } })), _jsxs("div", { className: "cadvp-read", children: [_jsx("span", { className: "xy", children: coords ? `X ${fmt(coords.x)}  Y ${fmt(coords.y)}${units ? ` ${units}` : ""}` : "X —  Y —" }), hint && _jsx("span", { className: "hint", children: hint })] })] }));
});
...
function SnapGlyph({ type, x, y }) {
    const c = "#a3e635";
    const r = 6;
    const common = { fill: "none", stroke: c, strokeWidth: 1.6 };
    switch (type) {
        case "end": return _jsx("rect", { x: x - r, y: y - r, width: r * 2, height: r * 2, ...common });
        case "mid": return _jsx("polygon", { points: `${x},${y - r} ${x + r},${y + r} ${x - r},${y + r}`, ...common });
        case "center": return _jsx("circle", { cx: x, cy: y, r: r, ...common });
        case "intersection": return (_jsxs(_Fragment, { children: [_jsx("line", { x1: x - r, y1: y - r, x2: x + r, y2: y + r, stroke: c, strokeWidth: 1.6 }), _jsx("line", { x1: x - r, y1: y + r, x2: x + r, y2: y - r, stroke: c, strokeWidth: 1.6 })] }));
        case "perp": return _jsx("path", { d: `M ${x - r} ${y - r} L ${x - r} ${y + r} L ${x + r} ${y + r}`, ...common });
        case "node": return (_jsxs(_Fragment, { children: [_jsx("circle", { cx: x, cy: y, r: r, ...common }), _jsx("line", { x1: x - r, y1: y - r, x2: x + r, y2: y + r, stroke: c, strokeWidth: 1.2 }), _jsx("line", { x1: x - r, y1: y + r, x2: x + r, y2: y - r, stroke: c, strokeWidth: 1.2 })] }));
        case "nearest": return _jsx("polygon", { points: `${x - r},${y - r} ${x + r},${y - r} ${x - r},${y + r} ${x + r},${y + r}`, ...common });
        default: return _jsx("polygon", { points: `${x},${y - 5} ${x + 5},${y} ${x},${y + 5} ${x - 5},${y}`, fill: c });
    }
}
