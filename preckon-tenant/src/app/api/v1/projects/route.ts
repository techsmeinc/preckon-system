...
export const GET = route(async (_req, ctx) => {
  ...
  const rows = ctx.permissions.has("project.read_all")
    ? await query(
        "SELECT id, name, code, client_name, status, lifecycle_key, lifecycle_state, created_at FROM project WHERE tenant_id = ? AND status <> 'archived' ORDER BY created_at DESC",
        [ctx.tenantId]
      )
    : await query(
        `SELECT p.id, p.name, p.code, p.client_name, p.status, p.lifecycle_key, p.lifecycle_state, p.created_at
           FROM project p JOIN project_member m ON m.project_id = p.id
          WHERE p.tenant_id = ? AND m.user_id = ? AND p.status <> 'archived' ORDER BY p.created_at DESC`,
