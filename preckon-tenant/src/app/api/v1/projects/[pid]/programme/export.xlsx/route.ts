...
export const POST = route<{ pid: string }>(async (req, ctx, { pid }) => {
  ...
  const legendRow = r + 1;
  ws.getCell(legendRow, 1).value = "◆ = milestone";
  ws.getCell(legendRow, 1).font = { size: 8, color: { argb: CRIT_EDGE } };
  ws.getCell(legendRow, 3).value = "Red edge = critical path";
  ws.getCell(legendRow, 3).font = { size: 8, color: { argb: CRIT_EDGE } };
  ws.getCell(legendRow, 6).value = `Generated by Preckon · ${new Date().toISOString().slice(0, 10)}`;
  ws.getCell(legendRow, 6).font = { size: 8, italic: true, color: { argb: "FF757575" } };
