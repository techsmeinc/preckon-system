...
The platform is fully specified: a domain-neutral **Core** (framework v1.3), a complete first product (**construction pack** v1.3) including tender management and a Standards & Rules engine (v2 — tier precedence + validation), a **second pack** (underwriting) that proves a new vertical needs no Core change, the frontend data path, the Host seam, per-agent implementation specs for the first build slice, and a ticketed build plan.
...
The construction pack also carries the **Standards & Rules engine** (v1.2, §3) — the platform's third strategic asset, added as Library data + a service agent with zero Core change. Everything else is done.
...
4. **Construction pack** — `preckon-construction-pack-design.md` (v1.3). The first product: agents, workflows, artifact types, personas, the bid-pursuit lifecycle, and the Standards & Rules engine (§3); full Standards design in `preckon-standards-rules-capability.md`.
...
| File | What |
|---|---|
| `preckon-tenant-platform-design.md` / `.docx` | **Core / framework, v1.3** — the engine. Domain-neutral (adds the §M.5 standards-resolution mechanism). |
| `preckon-construction-pack-design.md` / `.docx` | **Construction pack, v1.3** — the first product (tender management + Standards & Rules v2: tiers, validation, `standard_violation`). |
...
| `preckon-standards-rules-capability.md` | Standards & Rules capability **v2** — tiered `standard_rule`, resolve + validate (`standard_violation`), and licensing/provisioning modes (§10). Folded into pack §3; Core mechanism at framework §M.5. |
...
- **Both packs add zero Core DDL** (no `create/alter table`) — the "same Core, second vertical" proof. The Standards & Rules engine adds no tables either: a thin Core resolver (framework §M.5) over pack/Library rule content; tier precedence and the `standard_violation` schema validate.
...
5. **Standards & Rules** — tiers + validation ship (pack v1.3, capability v2). Future: the **deterministic Rules-Engine sweep** (execute `applies_when → result` without an LLM) and **Preckon-licensed standards data packs** (capability §10, mode 3). Launch runs license-clean (mode 1) + customer-uploaded licenses (mode 2) — no standards spend required.
