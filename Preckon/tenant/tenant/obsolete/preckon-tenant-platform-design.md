...
**Version:** 0.9 (framework complete — §§0–D + §X cross-cutting; Construction pack pending)
**Status:** Preckon Core — IAM (§1), the three keystones (§§2–4), AI orchestration (§5), the Orchestrator/Copilot (§6), ingestion + retrieval (§7), entitlements (§8), audit + observability (§9), Memory/flywheel (§M), domain packs (§D), cross-cutting conventions (§X). Modules/workflows detail is a companion doc, written after the skeleton proves the ABI.
...
## §6 — The Orchestrator role (Core) + Construction Copilot (userland)

The supervisor. Two layers, split on the domain line: Core provides a **generic supervisor contract** (§6.1, no construction knowledge); **Construction Copilot** is the userland agent that fills it (§6.2, all construction judgment).
...
`agent.construction_copilot`, `kind = supervisor`, is the one instance that fills the Orchestrator role today. Everything construction lives here; Core knows none of it.
...
Its full contents are the appendices, now read as **the Construction pack**: Appendix A the agents, Appendix B the workflows + the TenderLogix DAG, Appendix C the artifact types + ER + Host map. Everything there is pack data against the finished, generic ABI.
...
This chunk is Preckon Core itself, and every table on it is on the §S skeleton path. In build order, the next chunks are:

1. **Appendices A/B/C — the Construction pack** (§D.6): Appendix A the agents (all 13, I/O typed), Appendix B the workflows (seven Logix + the TenderLogix DAG), Appendix C the artifact types + ER reconciliation + Host workflow↔module map.
