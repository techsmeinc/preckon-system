...
**Version:** 1.0 (complete — §0 scope + manifest, §1 personas, Appendices A–C)
**Status:** the first domain pack, authored against **Preckon Core v1.0** (the finished, generic ABI). Complete: the manifest (§0.4), the persona roster (§1), the agents (Appendix A), the workflows (Appendix B), and the artifact types + ER reconciliation + Host map (Appendix C).
...
The **Construction pack**: the concrete instantiation that turns generic Preckon Core into a working preconstruction product. It is nothing but pack data (§D) — a single declared `domain` manifest plus the assets it names: an artifact-type vocabulary, fifteen agents (eleven workers, a Knowledge service, and three supervisor personas), seven workflows, a role template, and library collections. Core loads it without modification; adding this pack (or a legal-review pack tomorrow) is a data change, not an engineering change. That property is the whole point of §D, and this deck is the proof that Core earned it.
...
| **permission additions** — beyond the Core 18 | §1.2 catalog | none in v1 (Core's 18 suffice) |
...
```jsonc
{
  ...
  "agents": [
    ...
    "construction.agent.compliance", "construction.agent.proposal", "construction.agent.knowledge",
...
The distinctions that carry the product: **estimator** runs workflows but **qs_reviewer** only confirms their output; **precon_lead** is project admin without tenant admin; **owner** alone transfers ownership. Tiers: `owner_admin` = {owner, admin}, `delivery` = {precon_lead, estimator}, `review` = {qs_reviewer}, `view` = {viewer}.
...
- **Construction Copilot / Bid-Manager role** (default, whole-pursuit). Scope `{}` — the entire run/project; the full deviation set. Voice: owns the pursuit end to end — keeps the chain moving, surfaces blockers, and cross-checks that the priced proposal reconciles with the BOQ, quantities are sane against takeoff, and no spec scope is missing. The customer-facing chat product and general colleague; the §6.2 whole-run supervisor. Jobs: `copilot.respond`, `copilot.review_run`.
...
1. ~~Appendix A — the agents (fifteen, I/O-typed).~~ **Done** (below).
2. ~~§1 — the persona roster (three colleagues).~~ **Done** (above).
3. ~~Appendix B — the workflows (seven Logix + the TenderLogix walking-skeleton DAG).~~ **Done** (below).
4. ~~Appendix C — artifact types + ER + Host map.~~ **Done** (below).

**The pack is complete** (Appendix C.5): five artifacts against a finished, unchanged Core.

---

## Appendix A — Agents

The fifteen agents that make Core a construction product — eleven workers, the Knowledge service, and three supervisor **personas** (§1) — authored against the finished ABI (§3) — pack data, not Core. Each is one row in the `agent` registry (§3.1): a `kind`, typed I/O over the artifact-type vocabulary (§2.1; full schemas in Appendix C), the AI `job_types` it enqueues (§5), and its gating. Keys shown are short forms of their `construction.`-namespaced values (§D.3).
...
- **Proposal** — `tender_summary`, `boq_line`, `cost_line`, `schedule_activity`, `procurement_package` **→ `proposal_doc`** · *`proposal.assemble` (deep)* · reviewable. Assembles the confirmed graph into the submittable bid; the chain's terminal artifact. Compliance runs **after** assembly (Appendix B.2): `proposal_doc` feeds Compliance, not the reverse — that is what keeps the DAG acyclic.
...
- **No new Core surface.** Every agent above is expressible with the existing four syscalls (supervisor personas via the §6.1 context/result shape — still no fifth syscall), the existing `agent` columns, and the twelve seeded artifact types. Adding the eighth workflow product later is another registry row, not a Core change — the property §D exists to guarantee.
...
### B.2 TenderLogix — production

Tender analysis + bid assembly. The BOQ/estimate/programme/packages are their own workflows (B.5–B.8) writing the shared graph; here **Proposal** reads their confirmed outputs and assembles the bid, then **Compliance** checks the assembled bid's coverage.
...
```json
{
  ...
  "definition": {
    "nodes": [
      ...
      { "id": "tender",     "kind": "agent", "agent_key": "agent.tender" },
      { "id": "gate_scope", "kind": "gate",  "gate_types": ["tender_summary"] },
      { "id": "rfi",        "kind": "agent", "agent_key": "agent.rfi" },
      { "id": "proposal",   "kind": "agent", "agent_key": "agent.proposal" },
      { "id": "compliance", "kind": "agent", "agent_key": "agent.compliance" },
      { "id": "gate_bid",   "kind": "gate",  "gate_types": ["proposal_doc", "compliance_item"] }
    ],
    "edges": [
      { "from": "ingest", "to": "tender" },
      { "from": "tender", "to": "gate_scope" },
      { "from": "gate_scope", "to": "rfi" },
      { "from": "gate_scope", "to": "proposal" },
      { "from": "proposal", "to": "compliance" },
      { "from": "compliance", "to": "gate_bid" }
    ]
  }
}
```

`proposal` reads `tender_summary` (upstream) plus `boq_line`, `cost_line`, `schedule_activity`, `procurement_package` from the shared graph (B.5–B.8). `rfi` is an advisory branch off the confirmed scope — it emits clarification `rfi`s for the team, not a proposal input. `gate_bid` is where the **Compliance Lead** persona (§1) reviews coverage before submission.
...
Every piece Core needs to become a working preconstruction product is now specified as data against the finished ABI: the manifest (§0.4), the persona roster (§1), the agents (Appendix A), the workflows (Appendix B), and the types + ER + Host map (Appendix C). It adds **no Core table, endpoint, syscall, or column** — the §D promise, cashed out end to end. A second pack (legal review, underwriting, diligence) is the same five artifacts against the same Core.
