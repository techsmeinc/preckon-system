...
**Version:** 1.3 (generic framework). Domain-neutral; each domain pack is a separate implementation deck. §6.4 the persona roster; §1.6 a generic project-lifecycle field; §M.5 a generic standards-resolution mechanism (rules are pack/Library data).
...
create table project (
  ...
  lifecycle_key      text,                          -- a pack-declared lifecycle (§1.6); null = none
  lifecycle_state    text not null default 'start', -- opaque current state; Core never interprets it
...
### §1.6 Project lifecycle — a generic state field

A project can carry an **opaque lifecycle state** so a pack can model a stateful process over it — a bid pursuit, a case, a claim — **without Core learning the process**. Core stores the state and validates transitions against a *pack-declared* state machine; it never knows what a state means. This is the §D boundary applied to process, not just data.

The `project` columns (§1.4): `lifecycle_key` (which pack lifecycle; null = none), `lifecycle_state` (opaque, default `start`), `lifecycle_state_at`.

The machine is **pack data** — a `lifecycles` entry in the domain manifest (§D.1): transitions of the form `{ from, trigger_type, trigger_match?, required_permission, to, terminal? }`. Core reads it; the pack owns it.

Transitions are **human-driven and audited.** When a human confirms an artifact (§2.6), the runtime calls the generic `advanceLifecycle(project, artifact)`: if `lifecycle_key` declares a transition from the current `lifecycle_state` whose `trigger_type` matches the confirmed artifact's type (and `trigger_match` its payload, when set) and the actor holds `required_permission`, Core sets the new state, stamps `lifecycle_state_at`, and writes a `pursuit.transitioned` audit event. No matching transition → no-op; the confirm still lands. A supervisor persona (§6.4) may *propose* the next move; only this confirm-driven path advances state — **propose-vs-dispose, at the lifecycle level.** Core validates *transition validity*, never *meaning*.

| Method | Path | Permission | Side effects |
|---|---|---|---|
| GET | `/projects/{pid}/lifecycle` | `project.read` | current `lifecycle_state` + transitions available to the caller |
...
- **lifecycles** — the project state machine(s) (§1.6): states + transitions Core validates against, meaning owned by the pack
- **standards** — `standard_rule`s in a Library collection (§M.5): tiered, executable rules Core resolves/validates, content owned by the pack
...
// domain.manifest for the Construction pack
{
  ...
  "personas":       ["construction.agent.construction_copilot", "construction.agent.commercial", "…"],
  "lifecycles":     [ { "key": "bid_pursuit", "start": "received", "transitions": [ /* … §1.6 shape … */ ] } ],
...
### §M.5 Standards resolution — a generic mechanism over the Library

Core provides a **domain-neutral** way to resolve and validate *standards* held as Library data — the same split as §1.6: the machine is Core, the content is pack data. A pack ships rules as `standard_rule` entries in a Library collection (§M.1); Core resolves and evaluates them **without knowing what any rule means**.

**The `standard_rule` contract** (generic; instances are pack content): `{ rule_id, standard, category, tier, binding, jurisdiction, subject, applies_when, result, exceptions[], evidence_required[], source_ref, status }`. `tier ∈ statutory ▸ industry ▸ client ▸ company ▸ project` (broadest → most-specific); `binding ∈ mandatory | default`. Core reads the shape; the pack owns the content.

**Resolution** — `resolveStandards(context, { category, subject, … }) → rule`: mandatory rules are hard constraints (broadest authority wins a direct conflict; compatible constraints stack); among defaults the **most-specific tier wins**; a default contradicting a mandatory is void and surfaced. Deterministic — no LLM.

**Validation** — `validateStandards(artifact) → violations`: evaluate applicable mandatory rules' `applies_when → result` against a confirmed artifact; each mismatch is a `standard_violation` (generic shape `{ rule_id, subject_artifact_id, observed, expected, severity, recommendation, status }`). Core evaluates; the calling agent emits (Core never emits, §5.1).

**Tools.** Worker agents reach the mechanism through two shared tools (Doc 2): `standards_lookup` (resolve) and `standards_validate` (validate). A high/`critical` open violation may gate a lifecycle transition (§1.6).

**Boundary.** This is pure mechanism — tier arithmetic + predicate evaluation, not one domain term. Every rule instance (building codes, measurement standards, company/client/project preferences) is pack/Library data. Core gains a resolver, never a standard.
...
| Lifecycle / pursuit | an opaque state a project carries (§1.6), advanced only by a human confirming a gating artifact; the state machine (states + transitions) is pack data, so Core models process without learning the domain. A construction *bid pursuit* is one such lifecycle |
| Standards engine | Core's domain-neutral resolver/validator (§M.5) over `standard_rule`s held as pack/Library data: tier precedence (statutory ▸ … ▸ project), `mandatory` vs `default`, and validation into `standard_violation`s. The mechanism is Core; every rule (codes, NRM, company/client/project preferences) is pack content |
