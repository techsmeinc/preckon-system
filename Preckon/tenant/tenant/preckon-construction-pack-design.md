...
**Version:** 1.3 (complete — §0 manifest, §1 personas, §2 bid-pursuit lifecycle, §3 Standards & Rules v2 [tiers + validation], Appendices A–C)
...
The **Construction pack**: the concrete instantiation that turns generic Preckon Core into a working preconstruction product. It is nothing but pack data (§D) — a single declared `domain` manifest plus the assets it names: an artifact-type vocabulary, twenty agents (fifteen workers, a Knowledge and a Standards service, and three supervisor personas), eleven workflows, a bid-pursuit lifecycle, a Standards & Rules engine (§3), a role template, and library collections. Core loads it without modification; adding this pack (or a legal-review pack tomorrow) is a data change, not an engineering change. That property is the whole point of §D, and this deck is the proof that Core earned it.
...
| **standards** — executable `standard_rule`s (seeded) | §M Library | **§3** |
...
```jsonc
{
  ...
  "artifact_types": [
    ...
    "construction.standard_violation"
  ],
  "agents": [
    ...
    "construction.agent.knowledge", "construction.agent.standards",
  ...
  "library_collections": ["rate_book", "standard_rule", "precedent_bid", "template"],
  "lifecycles": [ { "key": "bid_pursuit", "start": "received", "transitions": [ /* … see §2.1 … */ ] } ],
  "standards": [ { "collection": "standard_rule", "seed": "construction.standards.v1", "note": "license-clean baseline (code requirements-as-facts + company templates); licensed standards (NRM2/MasterFormat) are add-on packs — capability §10" } ],
  "role_template": [
...
Agents **ask before deciding**. Rather than hardcode standards or re-read PDFs, the pack ships standards as executable `standard_rule` objects that a **Standards service** resolves and worker agents query. The resolution/validation *mechanism* is a thin domain-neutral Core capability (framework §M.5); the *rules* are pack/Library data. Full design: `preckon-standards-rules-capability.md`.

### §3.1 The `standard_rule` object

A structured rule (generic contract: framework §M.5): `{ rule_id, standard, category, tier, binding, jurisdiction, version, subject, applies_when, result, exceptions[], evidence_required[], confidence_threshold, source_ref, status }`. `category` ∈ measurement / classification / code / drawing / material / safety / cost / contract / quality / environmental; **`tier`** ∈ statutory ▸ industry ▸ client ▸ company ▸ project; **`binding`** ∈ mandatory | default; `result` is category-shaped — measurement → `{measure_by, unit}`, classification → `{code, system, title}`, code → `{requirement, mandatory}`. Schema + rules across tiers validated in the capability spec.

### §3.2 The tier hierarchy

Rules resolve **deterministically** by tier — **statutory ▸ industry ▸ client ▸ company ▸ project** (broadest → most-specific), each `mandatory` or `default`. Mandatory rules are hard constraints (broadest authority wins a direct conflict; compatible constraints stack); among defaults the **most-specific tier wins**; a default contradicting a mandatory is void and flagged. So *company says m², statute says m³ → m³*, and *project preference beats company* where no statute dictates. The default seed is **license-clean** — company/client/project templates and code requirements-as-facts (the `standards` bundle §0.4, seeded §D.4); **licensed industry standards (NRM2, MasterFormat) are provisioned separately** — customer-uploaded under the client's own license, or a future Preckon-licensed add-on pack (capability spec §10). `company / client / project` rules are the tenant's own. Company standards are the moat; the flywheel (§M) proposes new ones from confirmed corrections.

### §3.3 The service & tools

`agent.standards` (service, Appendix A), over the Core mechanism (§M.5), exposes two tools: **`standards_lookup`** (resolve a rule) and **`standards_validate`** (check an artifact → violations). Consulted before proposing: **BOQ** (measurement, classification), **Cost** (cost), **Specification** (classification), **Drawing** (drawing). **Compliance** and **Risk** additionally *validate* — running mandatory rules against confirmed artifacts and emitting `standard_violation`s (code, safety, material).

### §3.4 Cited, validated, versioned

Outputs cite the rules applied — `standard_refs: [rule_id@version]` on `boq_line` / `cost_line` / `drawing_measurement` / `compliance_item` (Appendix C.2); applying a matched rule is a strong confidence `agreement` signal (Doc 2 §1); a rule update marks dependents `stale` (§2.4). **Validation** emits `standard_violation` findings (Appendix C.2) — a high/`critical` open violation can gate `approving → submitted` (§2). **v1** looks up, applies, and validates on demand; **v2** is a deterministic Rules-Engine sweep executing `applies_when → result` without an LLM — where "execute NRM2, don't read it" becomes literal.
...
The twenty agents that make Core a construction product — fifteen workers, two services (Knowledge, Standards), and three supervisor **personas** (§1) — authored against the finished ABI (framework §3) — pack data, not Core. Each is one row in the `agent` registry (§3.1): a `kind`, typed I/O over the artifact-type vocabulary (§2.1; full schemas in Appendix C), the AI `job_types` it enqueues (§5), and its gating. Keys shown are short forms of their `construction.`-namespaced values (§D.3).
...
| `agent.compliance` | Compliance | worker | `compliance_item`, `standard_violation` | cross-cutting |
...
| `agent.risk` | Risk | worker | `risk`, `standard_violation` | RiskReview / cross-cutting |
...
| `agent.standards` | Standards | service | *(none — returns rules)* | all (queried before deciding, §3) |
...
- **BOQ** — `tender_summary`, `spec_clause`, `drawing_measurement` **→ `boq_line`** · *`boq.derive_lines` (deep)* · reviewable. Shared by TenderLogix (from `tender_summary`, §S) and QuantLogix (from measurements + clauses); defined once because I/O is workflow-independent (§2.1). Calls **Knowledge** for precedent and **`standards_lookup`** for units / classification codes / company descriptions (Standards & Rules capability).
...
- **Compliance** — `tender_summary`, `spec_clause`, `proposal_doc` **→ `compliance_item`** (+ `standard_violation` via `standards_validate`, §3) · *`compliance.check` (deep)* · reviewable. Checks each mandatory requirement is addressed, and validates the bid against code/quality standards before submission.
...
- **Risk** — `tender_summary`, `spec_clause`, `drawing_measurement`, `boq_line`, `cost_line` **→ `risk`** (+ `standard_violation` via `standards_validate`, §3) · *`risk.assess` (deep)* · reviewable. Registers commercial/technical/programme risks; validates work against code/material standards; feeds `bid_decision` and `bid_approval`; the artifact **Commercial** watches.
...
- **Standards** (service, §3) — query-driven over the Library `standard_rule` collection **→ resolved rules** · *`standards.lookup` (routing)* · emits no artifact. Exposes the `standards_lookup` tool; resolves company-over-pack precedence + effective version. BOQ/Cost/Specification/Drawing/Compliance/Risk call it before proposing (measurement, classification, code, drawing, material, safety).
...
| `standard_violation` | yes | Compliance / Risk (validate, §3) | *(a confirmed artifact + a mandatory rule)* | `rule_id`, `severity`, `expected`, `observed` |
...
{ "$id": "drawing_measurement", "type": "object", "additionalProperties": false,
  ...
  "properties": {
    ...
    "location": { "type": "string" }, "method": { "type": "string" }, "standard_refs": { "type": "array", "items": { "type": "string" } } } }
```
```json
{ "$id": "boq_line", "type": "object", "additionalProperties": false,
  ...
  "properties": {
    ...
    "trade": { "type": "string" }, "notes": { "type": "string" }, "standard_refs": { "type": "array", "items": { "type": "string" } } } }
```
```json
{ "$id": "cost_line", "type": "object", "additionalProperties": false,
  ...
  "properties": {
    ...
    "rate_source": { "type": "string" }, "rate_book_ref": { "type": "string" }, "standard_refs": { "type": "array", "items": { "type": "string" } } } }
```
```json
...
{ "$id": "compliance_item", "type": "object", "additionalProperties": false,
  ...
  "properties": {
    ...
    "note": { "type": "string" }, "standard_refs": { "type": "array", "items": { "type": "string" } } } }
```
```json
...
{ "$id": "standard_violation", "type": "object", "additionalProperties": false,
  ...
  "properties": {
    ...
    "observed": { "type": "object" }, "expected": { "type": "object" },
    "severity": { "type": "string", "enum": ["info","low","medium","high","critical"] },
    "reference": { "type": "string" }, "recommendation": { "type": "string" },
