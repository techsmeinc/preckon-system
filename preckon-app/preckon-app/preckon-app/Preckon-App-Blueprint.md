...
Overview · Documents · **Tender** (TenderLogix) · **Drawings** (DrawLogix) · **Specs** (DocLogix) · **BOQ** (QuantLogix) · **Estimate** (CostLogix) · **Schedule** (PlanLogix) · **Procurement** (ProcureLogix) · Activity/Audit — with the **Copilot** docked throughout.
...
- **Schedule — PlanLogix** — derive activity durations from BOQ quantities and output rates; sequence into a Gantt with a critical path; adjust a rate and the programme re-plans. Every bar traces to the BOQ lines that sized it.
...
- **Features** — the catalog editions draw from: the seven modules, Copilot, white-labeling, SSO, API access, usage limits — each a toggle or limit an edition can enable. Feature flags live here.
